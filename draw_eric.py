...
tf.flags.DEFINE_string("data_dir", "trace_dim", "directory to put model checkpoint")
...
A,B = 40, 200#200, 40 # image width,height
...
x = tf.placeholder(tf.float32,shape=(1,img_size)) # input (batch_size * img_size)
...
for t in range(T):
    ...
    write_h_dec, w_stats = write(h_dec)
...
if __name__ == '__main__':
    ...
    for i in range(train_iters):
        xtrain=train_data.next_batch(batch_size) # xtrain is (batch_size x img_size)
        feed_dict={x:xtrain}
